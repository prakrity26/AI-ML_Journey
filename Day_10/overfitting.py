...
# Unconstrained tree - likely overfitting
dt_overfit = DecisionTreeClassifier(random_state=42)
dt_overfit.fit(X_train, y_train)

# Constrained tree - max depth limits complexity
dt_constrained = DecisionTreeClassifier(max_depth=3, random_state=42)
dt_constrained.fit(X_train, y_train)

for name, model in [("Overfit Tree", dt_overfit), ("Constrained Tree", dt_constrained)]:
    train_acc = accuracy_score(y_train, model.predict(X_train))
    test_acc = accuracy_score(y_test, model.predict(X_test))
    print(f"\n{name}")
    print(f"Train: {train_acc:.2f} | Test: {test_acc:.2f} | Gap: {train_acc-test_acc:.2f}")